...
<nav class="menu-block">
	<ul>
		<li class="att popup-wrap">
			<a id="hd_singin_but_open" href="/login/" class="btn-toggle"><?=GetMessage("AUTH_LOGIN_LINK_TEXT")?></a>
			<form class="frm-login popup-block" name="system_auth_form<?=$arResult["RND"]?>" method="post" target="_top" action="<?=$arResult["AUTH_URL"]?>">
				...
				<div class="frm-row">
					<?if ($arResult["CAPTCHA_CODE"]):?>
								...
								<img src="/bitrix/tools/captcha.php?captcha_sid=<?echo $arResult["CAPTCHA_CODE"]?>" alt="CAPTCHA"><br><br>
...
<?}else{?>
	...
	<ul>
		...
		<li>
			<a href="
			<?echo $APPLICATION->GetCurPageParam("logout=yes", array(
				"login",
				"logout",
				"register",
				"forgot_password",
